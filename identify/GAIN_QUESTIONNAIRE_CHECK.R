...
#           variable hits      -> at least "review then reach out"
...
DISP_PAT <- regex(paste0(
  "refug|asylum|displac|\\bidps?\\b|stateless|forced to (leave|move|flee)|\\bfled\\b|\\bflee\\b|returnee|",
  "r[eé]fugi|d[eé]plac|apatrid|\\basile\\b|desplazad|refugiad|",
...
check_one <- function(base, path, id) {
  ...
  strong <- str_detect(vtxt, DISP_PAT)
  ...
  tibble(ddi_found = TRUE, n_var = length(vars), n_var_hits = sum(strong),
         ...
         abstract_hit = str_detect(abst, DISP_PAT))
}
...
res <- cat_rows %>% mutate(r = pmap(list(base, path, cat_id), check_one)) %>% unnest(r) %>%
  mutate(questionnaire_verdict = case_when(
    ...
    n_var_hits > 0                  ~ "displacement questions found",
